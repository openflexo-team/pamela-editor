...
public class ImportsAnnotation extends PamelaAnnotation {
    ...
    public List<String> getValue() {
        if (annotationSource.getProperty("value") == null) return new ArrayList<>();
        List<AnnotationValue> annotationValues = ((AnnotationValueList) annotationSource.getProperty("value")).getValueList();
        List<JavaAnnotation> javaAnnotations = annotationValues.stream().map(annotationValue -> (JavaAnnotation) annotationValue).collect(Collectors.toList());
        return javaAnnotations.stream().map(ann -> ((TypeRef) ann.getProperty("value")).getType().getCanonicalName()).collect(Collectors.toList());
    }
     ...
     *
     * @param className The canonical name of the class to import.
     * @return
     */
     ...
     *
     * @param className The canonical name of the class to remove.
     * @return
     */
