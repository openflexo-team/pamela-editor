...
import org.openflexo.pamela.editor.ui.ShowContextualMenuControl;
import org.openflexo.pamela.editor.ui.diagram.DrawEdgeControl;
import org.openflexo.pamela.exceptions.ModelDefinitionException;
import org.openflexo.pamela.factory.EditingContext;
import org.openflexo.pamela.undo.CompoundEdit;
import org.openflexo.pamela.undo.UndoManager;

public class ClassDiagramFactory extends DianaModelFactoryImpl {
	...
	public ClassDiagramFactory(EditingContext editingContext) throws ModelDefinitionException {
		super(ClassDiagram.class, Shape.class, Connector.class);
	...
	// Called for NEW
	public ClassDiagram makeNewDiagram() {
		CompoundEdit edit = getUndoManager().startRecording("Create empty diagram");
		ClassDiagram returned = newInstance(ClassDiagram.class);
	...
	public Connector makeNewConnector(ConnectorGraphicalRepresentation aGR, Shape from, Shape to, ClassDiagram classDiagram) {
		Connector returned = newInstance(Connector.class);
		returned.setName("Connector" + connectorIndex);
		connectorIndex++;
		returned.setDiagram(classDiagram);
	...
	public Connector makeNewConnector(Shape from, Shape to, ClassDiagram classDiagram) {
		return makeNewConnector(from, to, classDiagram, ConnectorType.LINE);
	}

	public Connector makeNewConnector(Shape from, Shape to, ClassDiagram classDiagram, ConnectorType connectorType) {
		Connector returned = newInstance(Connector.class);
		returned.setName("Connector" + connectorIndex);
		connectorIndex++;
		returned.setDiagram(classDiagram);
	...
	public Shape makeNewShape(ShapeType shape, DianaPoint p, ClassDiagram classDiagram) {
		ShapeGraphicalRepresentation gr = makeNewShapeGR(shape);
		gr.setWidth(100);
		gr.setHeight(80);
		return makeNewShape(gr, p, classDiagram);
	}

	public Shape makeNewShape(ShapeGraphicalRepresentation aGR, DianaPoint p, ClassDiagram classDiagram) {
	...
	public Shape makeNewShape(ShapeGraphicalRepresentation aGR, ClassDiagram classDiagram) {
		return makeNewShape(aGR, aGR.getLocation(), classDiagram);
	}
